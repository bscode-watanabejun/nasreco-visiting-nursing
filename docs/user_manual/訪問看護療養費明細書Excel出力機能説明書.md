...
| AE2 | レセプト種別（保険種別） | REレコードのレセプト種別コード（4桁）の2桁目を変換して表示<br>1 → 「1 社・国」（通常）または「1社保」（支払基金）<br>2 → 「2 公費」<br>3 → 「3 後期」<br>4 → 「4 退職」 | ✅ 実装済み |
| AH2 | レセプト種別（併用区分） | REレコードのレセプト種別コード（4桁）の3桁目を変換して表示<br>1 → 「1 単独」<br>2 → 「2 2併」<br>3 → 「3 3併」 | ✅ 実装済み |
| AK2 | レセプト種別（本人家族区分） | REレコードのレセプト種別コード（4桁）の4桁目を変換して表示<br>2 → 「2 本人」<br>4 → 「4 六歳」<br>6 → 「6 家族」<br>8 → 「8 高齢一」<br>0 → 「0 高齢7」 | ✅ 実装済み |
