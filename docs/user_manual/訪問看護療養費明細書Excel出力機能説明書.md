...
| AE2 | レセプト種別（保険種別） | 患者情報・保険証情報・公費情報から自動判定されたレセプト種別コード（4桁）の2桁目を変換して表示<br>保険者番号から詳細な保険種別を自動判定（8桁で'39'で始まる→後期高齢者医療、6桁→国民健康保険、8桁で'39'以外→社会保険）<br>1 → 「1 社・国」（通常）または「1社保」（支払基金）<br>2 → 「2 公費」<br>3 → 「3 後期」<br>4 → 「4 退職」 | ✅ 実装済み |
| AH2 | レセプト種別（併用区分） | 患者情報・保険証情報・公費情報から自動判定されたレセプト種別コード（4桁）の3桁目を変換して表示<br>公費負担医療証の登録数（0-4枚）から自動判定<br>1 → 「1 単独」<br>2 → 「2 2併」<br>3 → 「3 3併」 | ✅ 実装済み |
| AK2 | レセプト種別（本人家族区分） | 患者情報・保険証情報・公費情報から自動判定されたレセプト種別コード（4桁）の4桁目を変換して表示<br>保険証の続柄と患者の年齢から自動判定<br>2 → 「2 本人」<br>4 → 「4 六歳」<br>6 → 「6 家族」<br>8 → 「8 高齢一」<br>0 → 「0 高齢7」 | ✅ 実装済み |
...
| G8 | 枝番 | 資格確認レコードの枝番（被保険者証の枝番欄の番号、2桁、未記載の場合は空欄、後期高齢者医療の場合は省略） | ✅ 実装済み |
...
摘要欄には、以下の3つの項目が項目ごとに点線で区切って順に表示されます。各項目は該当する情報がある場合のみ表示されます。

| セル範囲 | 出力内容 | 説明 | 実装状況 |
|---------|---------|------|---------|
| P43:AL72 | 摘要欄全体 | ア・イ・ウの3項目を順に表示（最大15行） | ✅ 実装済み |

**ア. 第二～第四公費 公費給付対象一部負担金情報**

| セル | 出力内容 | 説明 | 実装状況 |
|------|---------|------|---------|
| S43以降 | 公費給付対象一部負担金情報 | 第2公費以降の公費給付対象一部負担金を表示（複数行で表示） | ✅ 実装済み |

**条件**: 公費が2件以上あり、2レコード目以降に公費給付対象一部負担金が存在する場合のみ表示
...
| セル | 出力内容 | 説明 | 実装状況 |
|------|---------|------|---------|
| S43以降 | 主治医情報 | 2人目以降の主治医情報を表示（複数行で表示） | ✅ 実装済み |

**条件**: 主治医が2人以上登録されている場合、2人目以降を表示
...
| セル | 出力内容 | 説明 | 実装状況 |
|------|---------|------|---------|
| P43以降 | レセプト表示欄 | 訪問看護療養費マスタの「レセプト表示欄」（2桁） | ✅ 実装済み |
| Q43以降 | レセプト表示項 | 訪問看護療養費マスタの「レセプト表示項」（2桁） | ✅ 実装済み |
| R43以降 | 負担区分 | KAレコードの負担区分 | ✅ 実装済み |
| S43以降 | 名称 | サービスコードマスタの「サービス名称」 | ✅ 実装済み |
| AG43以降 | 金額（円） | KAレコードの金額（基本療養費と加算を分けて集計） | ✅ 実装済み |
| AL43以降 | 日数（日） | 同一サービスコードのKAレコード数 | ✅ 実装済み |

**表示ルール**:
- 基本療養費と加算は別々の行に表示されます
- 基本療養費の金額はサービスコードマスタから取得した点数を使用（加算を含まない）
- 加算の金額は加算履歴から取得
- 同一の「レセプト表示欄」「レセプト表示項」が連続する場合は、2行目以降は省略されます
- 特別地域訪問看護加算（金額識別=5）の場合は、直前の基本療養費と連続表示されます
- 最大15行まで表示可能（15行を超える場合は警告を出力）
- テンプレートファイルでP43:P72、Q43:Q72、R43:R72、S43:S72、AG43:AG72、AL43:AL72が2行ずつ15組に結合されているため、各項目は2行分の高さで表示されます
...
レセプト種別コード（AE2、AH2、AK2セル）は、患者情報・保険証情報・公費情報から自動的に判定されます。

#### 出力内容

- **AE2（保険種別）**: レセプト種別コードの2桁目を変換して表示
  - 社会保険・国民健康保険の場合、提出先が支払基金の場合は「1社保」、その他の場合は「1 社・国」と表示されます
  - 公費のみの場合は「2 公費」
  - 後期高齢者医療の場合は「3 後期」
- **AH2（併用区分）**: レセプト種別コードの3桁目を変換して表示
  - 単独の場合は「1 単独」
  - 2種類併用の場合は「2 2併」
  - 3種類併用の場合は「3 3併」
- **AK2（本人家族区分）**: レセプト種別コードの4桁目を変換して表示
  - 本人・世帯主の場合は「2 本人」
  - 6歳未満の場合は「4 六歳」
  - 家族の場合は「6 家族」
  - 高齢受給者一般の場合は「8 高齢一」
...
レセプト種別コードは、以下の情報から自動的に判定されます：

1. **保険種別**: 保険証情報の保険者番号から自動判定されます
   - 保険者番号が8桁で「39」で始まる → 後期高齢者医療
   - 保険者番号が6桁 → 国民健康保険
   - 保険者番号が8桁で「39」以外 → 社会保険（健康保険組合など）
   - 保険証が登録されていない場合 → 患者情報の保険種別から判定（デフォルトは社会保険）
2. **公費の有無**: 公費負担医療証の登録数（0-4枚）
3. **本人家族区分**: 保険証の続柄（本人・家族・未就学者など）
4. **年齢区分**: 患者の年齢（6歳未満・高齢受給者など）

これらの情報が不足している場合や、判定に失敗した場合は、デフォルト値が使用されるか、該当セルは空欄のままになります。

### 情報欄（B22セル）の出力内容

B22セルには、以下の情報が自動的に出力されます：

#### ア: 主たる傷病名
- **主たる傷病名**: 訪問看護指示書の傷病名コード（ICD-10）と傷病名称を表示
  - 傷病名コードが`0000999`（未コード化傷病名）の場合は、傷病名称のみ表示
  - それ以外の場合は、傷病名コードと傷病名称の両方を表示

#### イ: 心身の状態
- **心身の状態**: レセプト詳細画面で入力した心身の状態のテキスト（必須項目）
- **基準告示第2の1に規定する疾病等の有無**: 訪問看護指示書の設定に基づいて表示（別表7、別表8、無）
...
**更新履歴**:
- 2025年12月7日: 「心身の状態」を必須項目に変更（検証ボタンでバリデーションエラーをチェック）
- 2025年12月7日: B22（情報欄）に「ア: 主たる傷病名」と「イ: 心身の状態」を追加実装
- 2025年12月7日: G8（枝番）の説明を更新（被保険者証の枝番欄の番号を記録、未記載の場合は空欄、後期高齢者医療の場合は省略）
- 2025年1月: 摘要欄の説明を実装に合わせて更新（名称フィールドの説明を修正）
- 2025年12月: レセプト種別コード出力機能（AE2、AH2、AK2）の実装を完了
- 2025年12月7日: 保険者番号から詳細な保険種別を自動判定するロジックを追加（後期高齢者医療・国民健康保険・社会保険の自動判定）
- 2025年12月: 摘要欄（P43:AL72）の実装を完了（ア、イ、ウの3項目）
