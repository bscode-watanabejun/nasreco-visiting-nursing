...
```
請求書_YYYYMM_患者氏名.xlsx
領収書_YYYYMM_患者氏名.xlsx
```

例：
- `請求書_202511_山田太郎.xlsx`（2025年11月分、患者氏名「山田太郎」の請求書）
- `領収書_202511_山田太郎.xlsx`（2025年11月分、患者氏名「山田太郎」の領収書）

**注意**: ファイル名に含まれる患者氏名は、姓と名を結合した形式（例：「山田太郎」）です。Windowsで使えない文字（`/ \ : * ? " < > |`）は自動的に除去されます。
