...
**最終更新日**: 2025年12月7日（JSレコードの心身の状態を必須項目に変更）
...
| 2 | レセプト番号 | ○ | ✅ 実装済み | 可変項目（複数レセプト時は連番、0埋めなし） |
...
| 7 | 職務上の事由 | - | ✅ 実装済み | 空欄 |
| 8 | 証明書番号 | - | ✅ 実装済み | **レセプト詳細画面**「一部負担金額・減免情報」セクション「証明書番号」（国保の場合のみ表示、3桁以内の数字） |
| 9 | 一部負担金額 | - | ✅ 実装済み | **レセプト詳細画面**「一部負担金額・減免情報」セクション「一部負担金額（円）」 |
| 10 | 減免区分 | - | ✅ 実装済み | **レセプト詳細画面**「一部負担金額・減免情報」セクション「減免区分」（別表9: 1=減額、2=免除、3=支払猶予） |
| 11 | 減額割合 | - | ✅ 実装済み | **レセプト詳細画面**「一部負担金額・減免情報」セクション「減額割合（%）」（減免区分が「減額」の場合のみ表示、0-100%） |
| 12 | 減額金額 | - | ✅ 実装済み | **レセプト詳細画面**「一部負担金額・減免情報」セクション「減額金額（円）」（減免区分が「減額」の場合のみ表示） |
...
| 5 | 実日数 | ○ | ✅ 実装済み | **訪問記録画面**の訪問日から自動集計（公費IDでフィルタして計算） |
| 6 | 合計金額 | ○ | ✅ 実装済み | **訪問記録画面**の点数から自動計算（公費IDでフィルタして計算） |
| 7 | 一部負担金額 | - | ✅ 実装済み | **レセプト詳細画面**「公費負担医療情報」セクション「一部負担金額・公費給付情報（KOレコード用）」の「一部負担金額（円）」（各公費ごとに入力、8桁以内の数字） |
| 8 | 公費給付対象一部負担金 | - | ✅ 実装済み | **レセプト詳細画面**「公費負担医療情報」セクション「一部負担金額・公費給付情報（KOレコード用）」の「公費給付対象一部負担金（円）」（各公費ごとに入力、6桁以内の数字） |
...
| 7 | 枝番 | △ | ✅ 実装済み | **保険証管理画面**「枝番（医療保険のみ）」（2桁、未記載の場合は空欄、後期高齢者医療の場合は省略）<br>保険者のみ記録、公費は空欄（自動判定） |
...
| 2 | 窓口負担額区分（別表26） | ○ | ✅ 実装済み | **レセプト詳細画面**「高額療養費適用状況（MFレコード用）」から選択（01=高額療養費現物給付あり（多数回該当を除く）、02=高額療養費現物給付あり（多数回該当）、未選択=00） |
| 3-32 | 予備1-30 | - | ✅ 実装済み | 空欄 |
| 33 | 予備31 | - | ✅ 実装済み | 空欄 |
...
| 7 | 主治医への直近報告年月日 | - | ✅ 実装済み | **訪問看護報告書管理画面**「報告日」から自動取得（レセプト対象月以前の最も新しい報告日、YYYYMMDD形式） |
...
| 2 | 心身の状態 | ○ | ✅ 実装済み | **レセプト詳細画面**「心身の状態（JSレコード用）」（必須項目、レセプト単位で入力） |
| 3 | 基準告示第2の1に規定する疾病等の有無 | ○ | ✅ 実装済み | **訪問看護指示書画面**「基準告示第2の1に規定する疾病等の有無」（別表13: 01=別表7、02=別表8、03=無、未選択時はデフォルト値03を出力） |
...
| 2 | 訪問開始年月日 | ○ | ✅ 実装済み | **訪問記録画面**「訪問日」（訪問記録を日付順にソートしてから最初の訪問記録を取得） |
| 3 | 訪問した場所1コード | ○ | ✅ 実装済み | **訪問記録画面**「訪問場所コード」（訪問記録を日付順にソートしてから最初の訪問記録を取得） |
...
| 2 | 算定年月日 | ○ | ✅ 実装済み | **訪問記録画面**「訪問日」（同じサービスコードの同日複数回の訪問は1つのKAレコードにまとめる） |
| 3 | 負担区分（別表22） | ○ | ✅ 実装済み | 患者・保険証・公費情報から自動判定 |
| 4 | 訪問看護療養費コード | ○ | ✅ 実装済み | **訪問記録画面**「サービスコード」（同じサービスコードの同日複数回の訪問は1つのKAレコードにまとめる） |
| 5 | 数量データ | ○ | ✅ 実装済み | **マスターデータ**「きざみ値計算識別」で判定（"1"の場合は"1"を記録、"0"の場合は記録しない） |
| 6 | 金額 | ○ | ✅ 実装済み | **訪問記録画面**「点数」× 10（同日複数回の訪問をまとめる場合でも、1回分の金額を記録） |
| 7 | 職種等 | ○ | ✅ 実装済み | **訪問記録画面**「職員資格コード」（同日複数回の訪問の場合、訪問回ごとの職種等コードを連続して記録。例: "0111"=1回目の01と2回目の11。マスターデータの職種区分が存在する場合のみ記録） |
| 8 | 同日訪問回数 | △ | ✅ 実装済み | **マスターデータ**「実施回数区分」で判定（"0"以外の場合のみ記録。1回目は省略、2回目="02"、3回目以上="03"） |
| 9 | 指示区分 | ○ | ✅ 実装済み | **マスターデータ**「訪問看護指示区分」または「特別訪問看護指示区分」で判定（訪問看護指示区分が"1"、"3"、"5"、または特別訪問看護指示区分が"2"、"4"、"6"の場合のみ記録。HJレコードの指示区分コードを使用） |
...
| 7 | 職種等 | - | ✅ 実装済み | 空欄（加算は通常省略） |
| 8 | 同日訪問回数 | - | ✅ 実装済み | 空欄（加算は通常省略） |
| 9 | 指示区分 | - | ✅ 実装済み | **マスターデータ**「訪問看護指示区分」または「特別訪問看護指示区分」で判定（訪問看護指示区分が"1"、"3"、"5"、または特別訪問看護指示区分が"2"、"4"、"6"の場合のみ記録。HJレコードの指示区分コードを使用） |
...
- **状況**: ✅ 実装済み
- **内容**: 一部負担金額、減免区分、減額割合、減額金額、証明書番号
- **実装内容**: 
  - レセプト詳細画面に「一部負担金額・減免情報（HOレコード用）」セクションを追加
  - 証明書番号（国保の場合のみ表示、3桁以内の数字）
  - 一部負担金額（円、任意入力）
  - 減免区分（別表9: 1=減額、2=免除、3=支払猶予）
  - 減額割合（減免区分が「減額」の場合のみ表示、0-100%）
  - 減額金額（減免区分が「減額」の場合のみ表示）
  - onBlur方式で入力フィールドからフォーカスが外れたときに自動保存
  - Selectコンポーネントは閉じたときに自動保存

### 3. KOレコード: 一部負担金額・公費給付対象一部負担金・実日数・合計金額
- **状況**: ✅ 実装済み
- **内容**: 一部負担金額、公費給付対象一部負担金、実日数、合計金額
- **実装内容**: 
  - レセプト詳細画面の「公費負担医療情報」セクションに「一部負担金額・公費給付情報（KOレコード用）」セクションを追加
  - 各公費カードごとに独立した入力フィールドを追加
    - 一部負担金額（円、8桁以内の数字、任意入力）
    - 公費給付対象一部負担金（円、6桁以内の数字、任意入力）
  - onBlur方式で入力フィールドからフォーカスが外れたときに自動保存
  - JSONB形式で`monthly_receipts.publicExpenseBurdenInfo`に保存（公費IDをキーとしたマップ）
  - CSV出力時に公費IDで対応する負担情報を取得して出力
  - **恒久対応**: 訪問記録テーブルに`publicExpenseId`フィールドを追加し、訪問記録画面で公費を選択可能に
  - CSV生成時に公費IDでフィルタして実日数・合計金額を正確に計算
  - 記録詳細画面に適用公費情報を表示
...
- **実装内容**: 
  - **レセプト詳細画面**に「高額療養費適用状況（MFレコード用）」セクションを追加
  - Selectコンポーネントで選択可能（高額療養費該当なし、高額療養費現物給付あり（多数回該当を除く）、高額療養費現物給付あり（多数回該当））
  - onBlur方式で入力フィールドからフォーカスが外れたときに自動保存
  - Selectコンポーネントは閉じたときに自動保存
  - 医療保険レセプトのみ表示
  - 確定済みレセプトは編集不可
  - CSV出力時に選択された値に応じてコードを出力（01または02、未選択の場合は00）

### 5. IHレコード: 主治医への直近報告年月日
- **状況**: ✅ 実装済み
- **内容**: 主治医への直近報告年月日
- **実装内容**: 
  - **訪問看護報告書管理画面**の「報告日」から、レセプト対象月以前の最も新しい報告日を自動取得
  - 同じ患者・同じ施設の報告書のみを対象として検索
  - YYYYMMDD形式（西暦8桁）でCSV出力
  - 報告日が存在しない場合は空文字列を出力
