...
async function comprehensiveDiffCheck() {
  console.log('🔍 開発環境と本番環境の包括的な差異確認\n');
  ...
  try {
    ...
    if (missingInProd.length > 0) {
      console.log(`   ⚠️  本番環境に存在しないテーブル（追加予定）: ${missingInProd.join(', ')}`);
    }
    ...
    if (soleraProd) {
      console.log(`\n   ✅ 本番環境で「訪問看護ステーションソレア春日部」を確認: ${soleraProd.name} (ID: ${soleraProd.id})`);
    } else {
    ...
    if (soleraProd) {
      ...
      const prodReceipts = await prodPool.query({
        text: `SELECT COUNT(*) as count, COUNT(*) FILTER (WHERE is_confirmed = true) as confirmed_count FROM monthly_receipts WHERE facility_id = $1`,
        values: [soleraId]
      });
      ...
      const devReceipts = soleraDev ? await devPool.query({
        text: `SELECT COUNT(*) as count, COUNT(*) FILTER (WHERE is_confirmed = true) as confirmed_count FROM monthly_receipts WHERE facility_id = $1`,
        values: [soleraDev.id]
      }) : { rows: [{ count: 0, confirmed_count: 0 }] };
      
      console.log(`   月次レセプト数:`);
      console.log(`      本番: 総数 ${prodReceipts.rows[0].count}件、確定済み ${prodReceipts.rows[0].confirmed_count}件`);
      console.log(`      開発: 総数 ${devReceipts.rows[0].count}件、確定済み ${devReceipts.rows[0].confirmed_count}件`);
    ...
    if (schemaChanges.length > 0) {
      ...
      console.log(`      - デプロイ時に npm run db:push を実行する必要があります`);
    } else {
    ...
    // コード変更の確認（Gitの状態）
    console.log(`\n   📝 コード変更の確認:`);
    console.log(`      - 最新のコミットを確認してください`);
    console.log(`      - 今回の変更: サービスコードフィルタリングの修正`);
    console.log(`      - 影響範囲: フロントエンドのみ（データベース変更なし）`);
    ...
    console.log('\n   📋 デプロイ前のチェックリスト:');
    console.log('      [ ] 本番環境のDATABASE_URLが正しく設定されている');
    console.log('      [ ] 本番環境のSESSION_SECRETが設定されている');
    console.log('      [ ] 本番環境のNODE_ENV=productionが設定されている');
    if (schemaChanges.length > 0) {
      console.log('      [ ] 本番環境で npm run db:push を実行する準備ができている');
    }
    console.log('      [ ] 本番環境のバックアップが取得されている（推奨）');
    
    console.log('\n   📋 デプロイ後の確認事項:');
    console.log('      [ ] アプリケーションが正常に起動する');
    console.log('      [ ] サービスコードプルダウンに加算コードが表示されない');
    console.log('      [ ] 既存のレセプトデータが正常に表示される');
    console.log('      [ ] ソレア春日部のデータが正常にアクセスできる');

    console.log('\n' + '─'.repeat(80));
    console.log('✅ 包括的な差異確認が完了しました\n');
